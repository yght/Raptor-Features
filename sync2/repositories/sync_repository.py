...
class SyncRepository:
    ...
    def __init__(self, db):
        ...
        self.sync = db.get_collection('sync', codec_options=codec_options)
    ...
    def get_entries(self, query={}, skip = 0, limit = 1000000, sort = 'logged asc' ):
        count = self.sync.count_documents(query, skip=skip, limit=limit)
        sort_tuple = repo_utils.process_sort_value(sort)
        entries = list(self.sync.find(query, skip=skip, limit=limit).sort(*sort_tuple))
    ...
    def insert_entry(self, entry):
        result = self.sync.insert_one(entry)
